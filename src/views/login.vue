  ...
  methods: {
    login(event) {
      ...
      .then(async (data) => {
        ...
        else if(this.$store.state.bot.status === 'off' && (!this.$store.state.bot.alphainsider || !this.$store.state.bot.broker || this.$store.state.allocation.length <= 0)) return this.$router.replace('/setup');
